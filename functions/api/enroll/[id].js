...
export async function onRequestGet(context) {
  ...
  const setoran = await db.prepare("SELECT s.id, s.track_id, s.status, s.created_at, (SELECT AVG(ry.score) FROM review_ayat ry WHERE ry.setoran_id = s.id) AS avg_score FROM setoran s WHERE s.user_id = ? ORDER BY s.created_at DESC LIMIT 10").bind(id).all();
