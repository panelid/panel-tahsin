...
export async function onRequestPost(context) {
  ...
  try {
    const { name, email, password, role, wa_number, ref, username } = await request.json()
    ...
    await db.prepare("INSERT INTO users (id, name, email, password_hash, role, wa_number, referral_code, referred_by, username) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
      .bind(id, name, email, pwHash, role, wa_number || null, referralCode, referredBy, uname).run()
    if (referredBy) {
