...
const { env, request } = context;
  ...
  if (request.method === 'POST') {
    ...
    if (b.uid && b.uid !== me) return json({ error: 'bukan akunmu' }, 403);
    if (b.bio !== undefined) {
      await db.prepare("UPDATE users SET bio = ? WHERE id = ?").bind(String(b.bio).slice(0, 200), me).run();
    ...
    await db.prepare(`UPDATE users SET ${b.field} = ? WHERE id = ?`).bind(val, me).run();
